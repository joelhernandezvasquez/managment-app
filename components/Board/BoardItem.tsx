...
export const BoardItem:FC<ItemProp>= ({item}) => {
  
   const {closeSideBar} = useSideBar();
   const {setCurrentBoard,currentBoardSelected} = useUiStore();

   const handleBoardItemClick = () =>{
    setCurrentBoard({_id:item._id,name:item.name});
   }

   return (
    <li className={`${item._id === currentBoardSelected._id && dashboard.board_items_active}`} 
