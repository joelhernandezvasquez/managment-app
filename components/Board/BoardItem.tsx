...
export const BoardItem:FC<ItemProp>= ({board}) => {
  
   const {closeSideBar} = useSideBar();
   const {setActiveBoard,getActiveBoard} = useUIStates();

   const setCurrentBoard = () =>{
    setActiveBoard({_id:board._id,name:board.name});
    setTimeout(()=>{
     closeSideBar();
    },300)
   }
   ...
   return (
    <li className={`${board._id === getActiveBoard()._id && dashboard.board_items_active}`} 
